...
export default function AIAssistantPage() {
  ...
  return (
    <div className="flex flex-col min-h-full bg-dash-bg">
      ...
      <main className="flex-1 flex flex-col p-4 sm:p-6 gap-4 min-h-0" style={{ maxHeight: 'calc(100vh - 56px)' }}>

        {/* Header */}
        <div className="bg-dash-card border border-dash-border rounded-xl p-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 min-h-0">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              ...
              <div
                className={`max-w-[88%] sm:max-w-[80%] px-4 py-2.5 rounded-2xl ${
                  msg.role === 'user'
